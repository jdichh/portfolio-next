...
export default function Header() {
  return (
    <header className="z-[99] relative w-full font-medium">
      <motion.div
        className="fixed left-1/2 border-b-2 theme-switch theme-switch top-0 h-16 w-full my-transition"
        ...
        >
          {links.map((link) => (
            <li key={link.hash} className="flex text-[#236660]">
              <Link
                href={link.hash}
                className="my-transition p-4 text-lg text-light-theme-text dark:text-dark-theme-text hover:text-[#236660] dark:hover:text-[#236660] rounded-sm"
              >
                {link.name}
              </Link>
            </li>
